...
│   │   │   └── page.tsx                   → Login page
│   │   └── callback/
│   │       └── route.ts                   → OAuth callback — exchanges code, sets session cookies
│   ├── dashboard/
...
│       ├── auth/
│       │   └── refresh/route.ts           → Access-token refresh, called by the browser InsForge client
│       ├── resume/
│       │   ├── generate/route.ts          → Generate base resume PDF from profile
│       │   └── extract/route.ts           → Extract profile data from uploaded resume PDF
├── proxy.ts                                → Next.js 16's middleware.ts — refreshes session, gates protected routes
├── agent/
...
├── actions/
│   ├── auth.ts                            → OAuth sign-in initiation + sign-out (Server Actions only)
│   ├── profile.ts                         → Profile save + update
│   └── jobs.ts                            → Job status updates
├── components/
│   ├── ui/                                → shadcn/ui components only
│   ├── auth/
│   │   ├── OAuthButtons.tsx               → Google/GitHub sign-in buttons (Server Component, bound Server Actions)
│   │   ├── OAuthSubmitButton.tsx          → Client subcomponent — pending state via useFormStatus
│   │   └── SignOutButton.tsx
│   ├── layout/
│   │   ├── Navbar.tsx
│   │   └── Footer.tsx
...
Access: authenticated users only, own files only. Bucket created with `isPublic: false` — InsForge bucket access is an all-or-nothing flag, not per-path RLS, so "own files only" is an app-level convention (always upload/read at `resumes/{user_id}/resume.pdf`), not DB-enforced.

---

## Database RLS & Triggers

- Row level security is **enabled on all four tables**, with select/insert/update/delete policies scoped to `auth.uid() = user_id` (`= id` on `profiles`). InsForge exposes Supabase-style `auth.uid()`/`auth.jwt()` for this.
- `profiles.id` is both the PK and the FK straight to `auth.users.id` — no surrogate key.
- A `handle_new_user()` trigger function (`AFTER INSERT ON auth.users`) auto-creates an empty `profiles` row (id, email only) on signup — app code (Feature 06+) only ever `update`s `profiles`, never needs an insert-or-upsert.
- A `set_updated_at()` trigger (`BEFORE UPDATE ON profiles`) keeps `updated_at` current, since PostgREST-style `update()` calls don't set it themselves.
- FK behavior: `ON DELETE CASCADE` from `profiles` down through `agent_runs`/`jobs`/`agent_logs`. `jobs.run_id` and `agent_logs.job_id` are nullable with `ON DELETE SET NULL` (URL-sourced jobs have no run; a log entry's related job is optional).
- CHECK constraints enforce the enum-like columns build-plan specifies: `agent_runs.status`, `jobs.source`, `jobs.match_score` (0–100), `agent_logs.level`.
- Schema is managed directly via InsForge's `run-raw-sql` MCP tool (no separate migration-file system in this project); all SQL is written idempotently so re-running it is safe.
...
- Public routes: /, /login
- `proxy.ts` (Next.js 16's `middleware.ts` equivalent) refreshes and checks session on every protected route
...
There is no separate `@insforge/ssr` package — SSR helpers ship as a subpath
of the main SDK: `@insforge/sdk/ssr` (plus the lighter `@insforge/sdk/ssr/middleware`
for proxy/middleware files). Both resolve `NEXT_PUBLIC_INSFORGE_URL` /
`NEXT_PUBLIC_INSFORGE_ANON_KEY` automatically — no need to pass them explicitly.

Three separate InsForge surfaces — never mix them:

```typescript
// lib/insforge-client.ts
// Browser-side — read-only auth surface (getCurrentUser, getProfile,
// getPublicAuthConfig). No signIn/signUp/signOut here by design.
import { createBrowserClient } from "@insforge/sdk/ssr";
export const insforge = createBrowserClient();

// lib/insforge-server.ts
// Server-side — used in Server Components to read the current user via
...
// actions/auth.ts
// Server Actions / Route Handlers only — the only place sign-in, sign-up,
// OAuth exchange, and sign-out run. Mutates auth cookies directly; never
// return raw tokens from these to the client.
import { createAuthActions } from "@insforge/sdk/ssr";
const auth = createAuthActions({ cookies: await cookies() });
```

OAuth is server-initiated: a Server Action calls
`auth.signInWithOAuth(provider, { redirectTo, skipBrowserRedirect: true })`,
stores the returned `codeVerifier` in a short-lived httpOnly cookie, and
redirects to `data.url`. The callback Route Handler reads the verifier
cookie and calls `auth.exchangeOAuthCode(code, codeVerifier)`, which sets
the real session cookies. See `app/(auth)/callback/route.ts` and
`actions/auth.ts`.

`proxy.ts` (Next.js 16 renamed `middleware.ts` → `proxy.ts`) calls
`updateSession()` from `@insforge/sdk/ssr/middleware` to refresh the
session on every request, then checks `DEFAULT_ACCESS_TOKEN_COOKIE`
presence to gate protected routes.
