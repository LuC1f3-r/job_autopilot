...
- `SearchControls` — top card: Job Title / Location `Input`s (controlled, both start empty), `FormButton` `variant="primary"` "Find Jobs" action with a loading state (`Loader2` spin + "Searching..."), success/error banner below rendered from the real `findJobs` Server Action result (`bg-success-lightest`/`Sparkles` on success, `bg-error/10`/`AlertCircle` on error). `"use client"` as of Feature 10 — calls `findJobs` directly (same pattern as `ProfileForm` → `saveProfile`), no intermediate fetch to `/api/agent/find`.
- `JobsTable` — filter bar (icon `Input` + two unlabeled `Select`s for match filter / sort) over a table (`COMPANY`/`ROLE`/`MATCH SCORE`/`SALARY EST.`/`SOURCE`/`DATE POSTED`/`DATE FOUND`) and a pagination bar. Match score renders as a small inline bar + percentage via a local `MatchScoreBar`/`matchScoreColor` — color scale is green (`bg-success`, ≥90%) / blue (`bg-info-medium`, 80–89%) / orange (`bg-warning`, <90%), matching the design mock exactly (don't reuse `ProgressRing`'s scale here, it's tuned for profile-completion percentages, not match scores). Feature 10 rebased this off the real `Job` type (`lib/job-types.ts`) — real rows passed down as a `jobs` prop from `app/find-jobs/page.tsx`'s server-side fetch (`select("*")`, ordered by `found_at` desc, no filter/sort/pagination logic — Feature 11 wires the filter bar, sort dropdowns, and pagination to real queries). Empty state (`Inbox` icon, "No jobs yet") shows when `jobs.length === 0`. SOURCE renders as a small pill (`SourceBadge`, `LinkIcon` + "Search"/"URL" label); ROLE links out to `source_url` when present. DATE POSTED/DATE FOUND both use a shared `formatRelativeDate` helper ("2 hours ago"/"Yesterday"/"4 days ago") — no date library in the project, this is the only place that needs one so far.
