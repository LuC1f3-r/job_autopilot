...
There is no `@insforge/ssr` package — SSR helpers ship as a subpath of the
main SDK: `@insforge/sdk/ssr` (full) and `@insforge/sdk/ssr/middleware`
(lightweight, for `proxy.ts`). Both auto-resolve `NEXT_PUBLIC_INSFORGE_URL`
/ `NEXT_PUBLIC_INSFORGE_ANON_KEY` — no need to pass them explicitly.

Three separate surfaces — never mix them:

```typescript
// lib/insforge-client.ts — browser context only, read-only auth
// (getCurrentUser, getProfile, getPublicAuthConfig — no sign-in/out)
...
```typescript
// lib/insforge-server.ts — Server Components, reads the session cookie
...
```typescript
// actions/auth.ts — the only place sign-in, sign-up, OAuth exchange, and
// sign-out run. Mutates auth cookies directly; never return raw tokens.
import { createAuthActions } from "@insforge/sdk/ssr";

const auth = createAuthActions({ cookies: await cookies() });
```

OAuth is server-initiated (not client-side): a Server Action calls
...
redirects to `data.url`. The callback Route Handler reads that cookie and
calls `auth.exchangeOAuthCode(code, codeVerifier)`, which sets the real
session cookies. See `actions/auth.ts` and `app/(auth)/callback/route.ts`.
